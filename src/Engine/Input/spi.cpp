...
SPI_Request *SPI_CreateRequest(void) {
	SPI_Request *req = (SPI_Request*)malloc(sizeof(SPI_Request));
...
void SPI_Init(SPI_Callback callback) {
	...
	ChangeClearRCnt(2, 0);
	InterruptCallback((IRQ_Channel)6, &_spi_poll_irq_handler);
	InterruptCallback((IRQ_Channel)7, &_spi_ack_irq_handler);
	ExitCriticalSection();
